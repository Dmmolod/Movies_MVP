...
final class NetworkManager {
    ...
    private let key = "api_key=6b17d488280ac0859560482490d1f60d"
    private let language = "language=ru"
    private let baseImageUrl = "https://image.tmdb.org/t/p/original"
        
    func getFilms(page: Int?,
                  category: FilmCategory,
                  _ completion: @escaping (Result<FilmsResponse, Error>) -> Void) {
        
        guard let url = URL(
            string: baseUrl + category.rawValue + "?" + key + "&" + language + (page != nil ? "&page=\(page!)" : "")
        ) else { return }
    ...
    func getImages(for id: String,
                   _ completion: @escaping (Result<FilmImagesResponse, Error>) -> Void) {
        
        guard let url = URL(string: baseUrl + id + "/images" + "?\(key)") else { return }
        
        URLSession.shared.dataTask(with: url) { data, response, error in
            guard let data = data, error == nil,
                  let filmImagesResponse = try? JSONDecoder().decode(FilmImagesResponse.self, from: data) else {
                completion(.failure(NetworkError.imageNotFound))
                return
            }
